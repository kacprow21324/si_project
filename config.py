...
class HarmonySearchConfig:
    ...
    def __init__(self, budget=100000, HMS=10, HMCR=0.7, PAR=0.3, iterations=100, seed=None):
        """
        ...
        Args:
            ...
            HMCR: Harmony Memory Considering Rate - prawdopodobieństwo wyboru z pamięci (domyślnie 0.7)
    ...
    def _memory_index_pool(self):
        """Zwraca unikalne indeksy akcji występujące w pamięci harmonii."""
        pool = set()
    ...
    def _generate_harmony_from_pool(self, index_pool):
        """Buduje harmonię korzystając tylko z indeksów dostępnych w podanej puli."""
        if not index_pool:
    ...
    def _improvise_new_harmony(self):
        """
        Tworzy nową harmonię wg reguły HMCR:
        - jeśli random < HMCR: wybór elementów tylko z pamięci HM,
        - w przeciwnym razie: pełne losowanie z całej przestrzeni.
        """
    ...
    def _harmony_cost(self, harmony_indices):
        """Liczy całkowity koszt harmonii."""
        return sum(self.stocks[idx]["lot_cost"] for idx in harmony_indices)

    def _harmony_tickers(self, harmony_indices):
        """Zwraca listę tickerów dla harmonii."""
        return [self.stocks[idx]["ticker"] for idx in harmony_indices]

    def print_harmony_memory(self, header="PAMIĘĆ HARMONII", max_tickers=8):
        """Wyświetla wszystkie rozwiązania przechowywane w pamięci HM."""
        print("\n" + "=" * 90)
        print(header)
        print("=" * 90)

        if not self.harmony_memory:
            print("(brak harmonii w pamięci)")
            print("=" * 90)
            return
    ...
    def run_harmony_search(self, show_memory_every=0, verbose=True):
        """
        Uruchamia iteracyjną część Harmony Search.

        Args:
            show_memory_every: co ile iteracji wypisywać pełną pamięć (0 = tylko koniec)
            verbose: czy wypisywać log iteracji
        ...
        for iteration in range(1, self.iterations + 1):
            ...
            if verbose:
                print(
                    ...
                    f"źródło={source:>6} | new_fit=${new_score:>9.2f} | "
            ...
            if show_memory_every > 0 and iteration % show_memory_every == 0:
                self.print_harmony_memory(header=f"PAMIĘĆ HARMONII PO ITERACJI {iteration}")
        ...
        if verbose:
            ...
            self.print_harmony_memory(header="KOŃCOWA PAMIĘĆ HARMONII")
...
if __name__ == "__main__":
    ...
    # Uruchom optymalizację i pokaż końcową pamięć
